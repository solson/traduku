...
bot = Cinch::Bot.new do
  configure do |c|
    ...
    @max_chain_length = Configru.max_chain_length
    @bing = BingTranslator.new(Configru.bing_api_key)
    BING_LANGS = @bing.supported_language_codes
  end
    ...
    when 'eo-sentence'
      ...
      if langs.count == 2 && LERNU_LANGS.include?(langs[0]) && LERNU_LANGS.include?(langs[1]) && (langs[0] == 'eo' || langs[1] == 'eo')
        ...
        langs.each_cons(2) do |fromlang, tolang|
          text = @bing.translate(text, :from => fromlang, :to => tolang)
        end
        m.reply(text, true)
      elsif langs.count == 2 && %w[det detect].include?(langs[0]) && BING_LANGS.include?(langs[1])
        detected_fromlang = @bing.detect(args)
        m.reply("(Detected as #{detected_fromlang}) " + @bing.translate(args, :from => detected_fromlang, :to => langs[1]))
      else
